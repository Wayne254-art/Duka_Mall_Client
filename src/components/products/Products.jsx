...
const Products = ({ title, products }) => {
    ...
    return (
        <div className='flex gap-8 flex-col-reverse bg-slate-100 px-5 py-4 rounded-md'>
            <Carousel
                autoPlay={false}
                infinite={false}
                arrows={false}
                            ...
                            <div key={i} className='flex flex-col justify-start gap-2'>
                                {
                                    p.map((pl, j) => <Link key={j} className='flex justify-start items-start' to='#'>
                                        <img className='w-[80px] h-[80px] object-contain' src={pl.images[0]} alt="images" />
                                        <div className='px-3 flex justify-start items-start gap-1 flex-col text-slate-600'>
                                            <h2 className='text-[13px]'>{pl.name?.slice(0, 40)}</h2>
                                            <span className='text-md font-bold'>Ksh.{pl.price.toLocaleString()}</span>
