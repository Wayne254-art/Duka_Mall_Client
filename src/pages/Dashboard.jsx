...
import { get_customer_installment_products } from '../store/reducers/installmentReducer'
...
const Dashboard = () => {
    ...
    const redirect_installment_page = () => {
        if (userInfo) {
            navigate(`/dashboard/installment-products`)
        } else {
            navigate(`/login`)
        }
    }
                        ...
                        <div className={` rounded-md z-50 md-lg:absolute ${filterShow ? '-left-4' : '-left-[360px]'} w-[270px] ml-4 bg-white`}>
                            <ul className='py-2 text-slate-600 px-4'>
                                ...
                                <li className='flex justify-start items-center gap-2 py-2 cursor-pointer'>
                                    <span className='text-xl'><GiPayMoney /></span>
                                    <div onClick={redirect_installment_page} className='block'>My Installments</div>
                                </li>
                                <li className='flex justify-start items-center gap-2 py-2'>
                                    <span className='text-xl'><BsChat /></span>
                                    <Link to='/dashboard/chat' className='block'>Chat Sellers</Link>
                                </li>
